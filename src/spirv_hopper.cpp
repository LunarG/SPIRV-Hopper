...
bool Hopper::Reflect() {
    ...
    for (uint32_t i = 0; i < module.capability_count; i++) {
        switch (module.capabilities[i].value) {
...
bool Hopper::CreateGraphicsPipeline() {
    ...
    if (!has_shader_tile_image) {
        ...
        for (uint32_t i = 0; i < input_attachments.size(); i++) {
            max_input_attachment_index = std::max(max_input_attachment_index, input_attachments[i]->input_attachment_index);
        }
        for (uint32_t i = 0; i < max_input_attachment_index + 1; i++) {
